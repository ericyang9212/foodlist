  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4">
        {/* 食物名 — 主角 */}
        <h3 className="text-[20px] text-gold-gradient font-medium tracking-wide leading-tight mb-2">
          {item.name}
        </h3>
        ...
        <div className="flex items-center gap-2 mb-2.5">
          ...
          {item.rating && (
            <span className="ml-auto text-[#c9a961] tracking-[0.3em] text-[10px]">
              {'★'.repeat(item.rating)}
            </span>
          )}
        </div>
        ...
        <div className="text-[11px] text-[#666] tracking-wider">
          {item.restaurants.length === 0 ? (
            <span className="text-[#444] italic">未指定店家</span>
          ) : (
            <span>
